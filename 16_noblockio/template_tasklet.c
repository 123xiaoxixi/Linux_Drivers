...
//应用程序中poll()/select()/epoll()函数都会调用到内核中poll()函数
static unsigned int imx6ull_poll(struct file *filp, struct poll_table_struct *wait) {
    unsigned int mask = 0;
    struct template_dev *dev = (struct template_dev *)filp->private_data;
    //poll_wait 函数不会引起阻塞，只是将应用程序添加到poll_table中
    poll_wait(filp, &dev->r_wait, wait);

    //判断是否可读
    if (atomic_read(&dev->releasekey)) {    //按键按下       
        mask = POLLIN | POLLRDNORM;         //POLLIN表示有数据可读，相应的还是POLLOUT/POLLERR...
    }
    return mask;
